...
class MazeGenerator(object):
    """
    迷宫生成器类的主要属性：
    - maze_data: 每个单元格的信息被编码为5维数组:
        前4维表示墙壁状态，1表示有墙, 0表示无墙
        第1位对应格子的上边, 第2位对应右边, 第3位对应下边, 第4位对应左边
        第5维存储从终点到该格子的最短距离
    """

    def __init__(self, maze_size=16, seed=None, maze_density=0.95):
        """
        初始化迷宫生成器
        参数:
    ...
    def move_robot(self, direction):
        ...
        # 检查是否是合法移动
        if direction in self.get_action_mask(self.robot['loc']):
            # 更新机器人位置
    ...
    def get_action_mask(self, position):
        """
        获取当前状态下的动作掩码，用于标识哪些动作是合法的。
        
        参数:
            position: 当前位置坐标元组 (x, y)
            
        返回:
            list: 合法动作列表，每个动作用字符表示 ['U', 'R', 'D', 'L']
            其中U表示向上，R表示向右，D表示向下，L表示向左
        """
        y, x = position[1], position[0]
        results = []
        # 检查四个方向，0表示无墙可以移动
        for i, action in enumerate(self.valid_actions):
            if self.maze_data[y, x, i] == 0:
    ...
    def generate_maze(self, maze_size):
        ...
        while True:  # 循环直到生成有效的迷宫
            # 修改maze_shape，添加第5个维度用于存储Dijkstra距离值
            maze_shape = maze_size + (5,)  # 现在是5维：4个方向的墙 + 1个距离值
            visited_cells = np.zeros(maze_size, dtype=np.int_)
            # 创建迷宫数据数组，初始化所有墙为1（有墙）
            maze_data = np.ones(maze_shape, dtype=np.int32)
            # 将距离值（第5维）初始化为最大整数值
            maze_data[:, :, 4] = np.iinfo(np.int32).max
            ...
            # 随机选择一个方向打开墙（设置为0表示无墙）
            open_direction = random.choice(possible_directions)
            maze_data[dest_y, dest_x, open_direction] = 0
            # 设置相邻格子的对应墙
            if open_direction == 0:  # 上
                maze_data[dest_y - 1, dest_x, 2] = 0
            elif open_direction == 1:  # 右
                maze_data[dest_y, dest_x + 1, 3] = 0
            elif open_direction == 2:  # 下
                maze_data[dest_y + 1, dest_x, 0] = 0
            elif open_direction == 3:  # 左
                maze_data[dest_y, dest_x - 1, 1] = 0

            # 3. 设置中心区域内部的连接（十字相通）
            # 左上和右上之间的墙
            maze_data[center - 1, center - 1, 1] = 0  # 左上格子的右墙
            maze_data[center - 1, center, 3] = 0  # 右上格子的左墙
            # 左上和左下之间的墙
            maze_data[center - 1, center - 1, 2] = 0  # 左上格子的下墙
            maze_data[center, center - 1, 0] = 0  # 左下格子的上墙
            # 右上和右下之间的墙
            maze_data[center - 1, center, 2] = 0  # 右上格子的下墙
            maze_data[center, center, 0] = 0  # 右下格子的上墙
            # 左下和右下之间的墙
            maze_data[center, center - 1, 1] = 0  # 左下格子的右墙
            maze_data[center, center, 3] = 0  # 右下格子的左墙
            ...
            while len(wall_list):
                ...
                if not is_center and (visited_cells[current_y, current_x] == 0 or \
                    ...
                    # 打通当前格子到相邻格子的墙（设置为0表示无墙）
                    maze_data[current_y, current_x, wall_direction] = 0
                    # 打通相邻格子到当前格子的墙
            ...
            if self.validate_maze(maze_data):
                # 计算并添加Dijkstra Map
                maze_data = self.calculate_dijkstra_map(maze_data)
                return maze_data
    ...
    def create_additional_paths(self, maze_data):
        ...
        def check_pillar_walls(y, x, direction):
            ...
            for py, px in pillar_points:
                ...
                temp_maze_data[y, x, direction] = 0
                opposite_direction = (direction + 2) % 4
                temp_maze_data[adjacent_y, adjacent_x, opposite_direction] = 0

                # 检查这四个格子之间是否至少有一面墙
                walls_exist = False
                # 检查垂直墙
                if temp_maze_data[cells[0][0], cells[0][1], 1] == 1 or \
                        temp_maze_data[cells[2][0], cells[2][1], 1] == 1:
                    walls_exist = True
                # 检查水平墙
                if temp_maze_data[cells[0][0], cells[0][1], 2] == 1 or \
                        temp_maze_data[cells[1][0], cells[1][1], 2] == 1:
                    walls_exist = True
    ...
    def validate_maze(self, maze_data):
        ...
        exits = []
        for direction in range(4):
            if maze_data[start_y, start_x, direction] == 0:  # 0表示无墙
            ...
            for test_exit in exits:
                ...
                for direction in exits:
                    if direction != test_exit:
                        # 关闭当前格子的出口（设为1表示有墙）
                        test_maze[start_y, start_x, direction] = 1
                        # 关闭相邻格子的对应入口
                        ...
                        if direction == 0:
                            ...
                            nx -= 1  # 左
                        test_maze[ny, nx, (direction + 2) % 4] = 1
            ...
            if valid_exits:
                ...
                for direction in exits:
                    if direction != keep_exit:
                        # 关闭当前格子的出口（设为1表示有墙）
                        maze_data[start_y, start_x, direction] = 1
                        # 关闭相邻格子的对应入口
                        ny, nx = start_y, start_x
                        if direction == 0:
                            ny -= 1  # 上
                            ...
                            nx -= 1  # 左
                        maze_data[ny, nx, (direction + 2) % 4] = 1
            else:
    ...
    def calculate_dijkstra_map(self, maze_data, start_point=None):
        """
        计算从指定起点到每个格子的最短距离
        参数:
            maze_data: 迷宫数据
            start_point: 起点坐标元组(x,y)，如果为None则使用终点作为起点
        返回:
            更新后的maze_data（包含距离信息）
        """
        # 初始化距离数组
        distances = np.full((self.maze_size, self.maze_size), np.iinfo(np.int32).max, dtype=np.int32)
        
        # 确定起点
        if start_point is None:
            # 使用终点作为起点
            start_x, start_y = self.destination[0], self.destination[1]
        else:
            # 使用指定的起点
        ...
        while not pq.empty():
            ...
            for direction in range(4):
                # 只有当这个方向没有墙时才能移动
                if maze_data[current_y, current_x, direction] == 0:  # 0表示无墙
                    ...
                    # 检查新位置是否在迷宫范围内
                    if 0 <= next_x < self.maze_size and 0 <= next_y < self.maze_size:
                        # 如果新的距离更短，更新距离
        ...
        # 将计算出的距离更新到maze_data的第5维
        maze_data[:, :, 4] = distances
        return maze_data
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if event.key == K_q:
                    ...
                    reached_destination = maze.move_robot('U')
                    moved = True
                elif event.key == K_RIGHT:
                    reached_destination = maze.move_robot('R')
                    moved = True
                elif event.key == K_DOWN:
                    reached_destination = maze.move_robot('D')
                    moved = True
                elif event.key == K_LEFT:
                    reached_destination = maze.move_robot('L')
                    moved = True
