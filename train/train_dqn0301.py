...
# 最初可以训练的版本，使用explored_map作为CNN的输入，与当前位置的2维坐标合并，在16个迷宫中收敛
class CustomCNN(BaseFeaturesExtractor):
    ...
    def __init__(self, observation_space: gym.spaces.Dict):
        ...
        # 直接计算卷积层输出维度
        # 经过3次MaxPool2d(2)，特征图尺寸变为原来的1/8
        # 最后一层卷积输出通道数为64
        conv_out_size = 64 * (self.maze_size // 8) * (self.maze_size // 8)
...
def make_env(size=None, seed=None, render_mode="human"):
    """
    创建训练环境的工厂函数
    ...
    def _init():
        ...
        env = Monitor(env)
        # 每次reset时都生成新的迷宫
        env.reset(seed=seed, options={'generate_new_maze': True})
        return env
    return _init
